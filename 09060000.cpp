...
int main(void) {
	...
	for(int i=0;i< info.biHeight;i++)
		for (int j = 0; j < info.biWidth; j++) {
			training_target[3 * i][j] = (double) lpImg[i * 3 * info.biWidth + j * 3];
			training_target[3*i+1][j] = (double)lpImg[i * 3 * info.biWidth + j * 3 + 1];
			training_target[3*i+2][j] = (double)lpImg[i * 3 * info.biWidth + j * 3 + 2];
		}

	//축소,확대가 진행된 이미지를 training point로 설정.
	for (int i = 0; i < info.biHeight; i++)
		for (int j = 0; j < info.biWidth; j++) {
			training_point[3*i][j] = (double)bili_changed->imageData[(info.biHeight - 1 - i) * 3 * info.biWidth + info.biWidth-1-j * 3];
			training_point[3*i+1][j] = (double)bili_changed->imageData[(info.biHeight - 1 - i) * 3 * info.biWidth + info.biWidth - 1 - j * 3 + 1];
			training_point[3*i+2][j] = (double)bili_changed->imageData[(info.biHeight - 1 - i) * 3 * info.biWidth + info.biWidth - 1 - j * 3 + 2];
		}
	...
	for (int epoch = 0; epoch <= MAX_EPOCH; epoch++)
	{
		...
		for (p = 0; p < NUM_TRAINING_DATA; p++)
		{
			...
			for (k = 0; k < NUM_OUTPUT; k++)
				error += abs((output[k] - training_target[p][k]))/255;// *(output[k] - training_target[p][k]);
		...
		if (epoch % 1== 0) printf("%d: %f\n", epoch, 1-(error/(NUM_OUTPUT*NUM_TRAINING_DATA)));
	}
	...
	for (int m = 0; m < 4; m++) {
		for (int n = 0; n < info.biWidth*info.biHeight; n++) {//m*info.biWidth*info.biHeight+3*n
			test_point[3*i][n] = (double)bili_big->imageData[m*info.biWidth*info.biHeight + 3 * n];
			test_point[3*i+1][n] = (double)bili_big->imageData[m*info.biWidth*info.biHeight + 3 * n+1];
			test_point[3*i+2][n] = (double)bili_big->imageData[m*info.biWidth*info.biHeight + 3 * n+2];
		}
