...
int main(void) {
	...
	for (int i = 0; i < info.biHeight; i++)
		for (int j = 0; j < info.biWidth; j++) {
			training_point[3*i][j] = (double)bili_changed->imageData[(info.biHeight - 1 - i) * 3 * info.biWidth + j * 3];
			training_point[3*i+1][j] = (double)bili_changed->imageData[(info.biHeight - 1 - i) * 3 * info.biWidth + j * 3 + 1];
			training_point[3*i+2][j] = (double)bili_changed->imageData[(info.biHeight - 1 - i) * 3 * info.biWidth +j * 3 + 2];
		}
